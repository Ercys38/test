      ...
      return `
        <article class="memory memory--moment ${offsetClass}">
          <div class="memory__moment-card texture-orientale">
            <span class="memory__moment-icon" aria-hidden="true">${icon}</span>
          </div>
...
function playMemoriesMusic() {
  const audio = document.getElementById("bgMusic");
  if (!audio) return;
  // Premier lancement uniquement : on saute directement au meilleur
  // passage plutôt que de partir du tout début du morceau.
  if (audio.currentTime === 0) {
    audio.currentTime = CONFIG.MUSIQUE_START_SECONDES || 0;
  }
  audio.play().catch(() => {
...
// Quand le morceau arrive à sa fin, on reboucle sur le même passage
// (celui choisi dans CONFIG.MUSIQUE_START_SECONDES) plutôt que de
// repartir du tout début à chaque boucle.
function initMusiqueLoop() {
  const audio = document.getElementById("bgMusic");
  if (!audio) return;
